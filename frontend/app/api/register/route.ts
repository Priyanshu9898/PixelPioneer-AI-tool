...
export const POST = async (req: NextRequest) => {
    ...
    if(password !== confirmPassword) {
        return NextResponse.json({message: "Password does not match", success: false}, {status: 400});
    }
    ...
    if(isUserPresent) {
        return NextResponse.json({success: false, "message": "User Already Present"}, {status: 500});
    }
    ...
    try{
        ...
        const response = NextResponse.json({message: "User created successfully", success: true, user: user}, {status: 201});
